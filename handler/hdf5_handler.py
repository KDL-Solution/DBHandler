...
class HDF5Handler(FileHandler):
    def __init__(self, db_path, mode="r", force=False, verbose=False, compression=None, compression_opts=None):
        ...
        if not str(db_path).endswith(".hdf5"):
            raise ValueError(f"Invalid file extension: '{self.db_path}'. Use '.hdf5' extension.")
        ...
        if mode == "r":
            ...
            self.file.create_dataset("images", (0,), dtype=self.dtui8, maxshape=(None,), compression=compression, compression_opts=compression_opts)
            self.file.create_dataset("labels", (0,), dtype=self.utf8, maxshape=(None,), compression=compression, compression_opts=compression_opts)
